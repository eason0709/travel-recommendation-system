...
# Cluster labels are now data-driven.
# They are generated by cluster_interpretation_soft.py and saved to:
# data/soft_cluster_label_summary.csv
#
# This avoids conflicts where a fixed manual label says "low budget" while
# the current heatmap shows the same cluster has high budget.

DEFAULT_CLUSTER_LABELS = {}
DEFAULT_CLUSTER_DESCRIPTIONS = {}
...
def get_cluster_label(cluster_id):
    ...
    if cluster_label_summary is not None and "cluster_label" in cluster_label_summary.columns:
        matched = cluster_label_summary[
            cluster_label_summary["cluster"].astype(int) == cluster_id
        ]
        if len(matched) > 0:
            return str(matched.iloc[0]["cluster_label"])

    return DEFAULT_CLUSTER_LABELS.get(
        cluster_id,
        f"Cluster {cluster_id}（尚未產生資料驅動標籤）"
    )
...
def get_cluster_description(cluster_id):
    ...
    if cluster_label_summary is not None and "description" in cluster_label_summary.columns:
        matched = cluster_label_summary[
            cluster_label_summary["cluster"].astype(int) == cluster_id
        ]
        if len(matched) > 0:
    ...
    return DEFAULT_CLUSTER_DESCRIPTIONS.get(
        cluster_id,
        "目前尚未產生此 cluster 的資料驅動語意解釋。請先執行 cluster_interpretation_soft.py。"
    )
